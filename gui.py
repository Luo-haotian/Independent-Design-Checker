...
class IDC_GUI:
    ...
    def setup_ui(self):
        ...
        ttk.Combobox(frame, textvariable=self.jurisdiction_var, values=["HK", "BS", "GB"], width=10).grid(row=6, column=2, sticky="w")
    ...
    def start(self):
        ...
        worker = threading.Thread(
            target=self.check,
            args=(pdf_path, self.type_var.get(), self.output_var.get(), self.model_var.get(), self.provider_var.get(), self.jurisdiction_var.get(), self.code_pack_var.get(), self.overrides_var.get(), self.critic_var.get(), self.json_var.get()),
        )
        worker.daemon = True
        worker.start()

    def check(self, pdf_path, structure_type, output_dir, model, provider, jurisdiction, code_pack, overrides, critic, export_json):
        try:
            ...
            with redirect_stdout(capture):
                success = checker.check(pdf_path, structure_type, output_dir, jurisdiction=jurisdiction, code_pack=code_pack, input_overrides=overrides or None, critic=critic, export_json=export_json)
