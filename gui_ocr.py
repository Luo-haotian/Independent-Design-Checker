"""GUI for IDC with OCR Support (Tesseract version)"""
...
from main_ocr import CheckerOCR, MODEL_CONFIGS, estimate_tokens, API_PROVIDER, TESSERACT_AVAILABLE
...
class IDC_GUI_OCR:
    def __init__(self, root):
        self.root = root
        self.root.title("IDC - Structural Verification (with OCR)")
    ...
    def setup_ui(self):
        ...
        # Title
        ttk.Label(frame, text="Independent Design Checker (IDC) with OCR", 
        ...
        if TESSERACT_AVAILABLE:
            ...
            ocr_color = "orange"
        ttk.Label(frame, text=f"Powered by {API_PROVIDER.upper()} AI | {ocr_status}", 
                 foreground=ocr_color).grid(row=1, column=0, columnspan=3, pady=(0, 10))
        ...
        model_combo = ttk.Combobox(frame, textvariable=self.model_var, 
                                   values=list(MODEL_CONFIGS.keys()), state="readonly", width=25)
        ...
        # Buttons
        btn = ttk.Frame(frame)
        btn.grid(row=10, column=0, columnspan=3, pady=10)
        ...
        if not TESSERACT_AVAILABLE:
            ...
            ttk.Label(frame, text=info_text, foreground="red", font=("Arial", 9)).grid(row=11, column=0, columnspan=3, pady=(5, 0))
        else:
            info_text = "Tip: Use 'Force OCR' for scanned documents, 'Auto' for mixed content, 'No OCR' for text-based PDFs"
            ttk.Label(frame, text=info_text, foreground="gray", font=("Arial", 9)).grid(row=11, column=0, columnspan=3, pady=(5, 0))
    ...
    def start(self):
        ...
        if not TESSERACT_AVAILABLE and self.ocr_var.get() != "no":
            self.log("WARNING: Tesseract not installed. OCR features disabled.")
        ...
        t = threading.Thread(target=self.check, args=(
            pdf, self.type_var.get(), self.output_var.get(), 
            self.model_var.get(), force_ocr, use_ocr
        ))
        t.daemon = True
        t.start()
    
    def check(self, pdf, stype, out, model, force_ocr, use_ocr):
        try:
            ...
            with redirect_stdout(buf):
                success = checker.check(pdf, stype, out, force_ocr=force_ocr)
